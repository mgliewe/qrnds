...
RED.editor = (function() {
	...
	function showEditDialog(node) {
		...
		let html = "";
		for (n in node._def.defaults) {
			let def = node._def.defaults[n];
			html += '<div class="form-row">'
			      + '<label for="node-input-' + n + '"><i class="fa fa-tag"></i> ' + n + '</label>'
				  + '<input type="text" id="node-input-' + n + '" placeholder="' + n + '"></input>'
				  + '</div>'
		}
		form.html(html);
		prepareEditDialog(node,node._def,"node-input");
		$( "#dialog" ).dialog("option","title","Edit "+node.type+" node")
					  .dialog( "open" );
