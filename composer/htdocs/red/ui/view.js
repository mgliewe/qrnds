...
RED.view = (function() {
	...
	var allow_multiple_inputs = true;
	var allow_multiple_outputs = false;
...
*/
	...
	var workspace_tabs = RED.tabs.create({
		...
		onadd: function(tab) {
			if (workspace_tabs.count() == 1) {
				RED.enable_menu("worksheet-delete", false)
			} else {
				RED.enable_menu("worksheet-delete", true)
			}
		},
	...
	function showConsole() {
		$("#node-dialog-console-terminal").show();
		RED.runtime.fit_terminal();
	}

	function hideConsole() {
		$("#node-dialog-console-terminal").hide();		
	}
...
//		workspace_tabs.activateTab(tabId);
	...
	function deleteSelection() {
		...
		if (moving_set.length > 0) {
			for (var i=0;i<moving_set.length;i++) {
				...
				for (var j=0; j < rmlinks.length; j++) {
					...
					if (link.target == node) {
						// reenable output port
						var n = link.sourcePort;
						var rect = link.source.outputlist[n];
						rect.on("mousedown", (function(nd,n){return function(d){portMouseDown(d,0,n);}})(link.source, n))
							.on("touchstart", (function(nd,n){return function(d){portMouseDown(d,0,n);}})(link.source, n))
							.on("mouseup", (function(nd,n){return function(d){portMouseUp(d,0,n);}})(link.source, n))
							.on("touchend", (function(nd,n){return function(d){portMouseUp(d,0,n);}})(link.source, n))
							.on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=RED.state.JOINING || mousedown_port_type != 0 ));})
							.on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);})
					}
				}
		...
		if (selected_link) {
			...
			// reenable output port
			var n = selected_link.sourcePort;
			var rect = selected_link.source.outputlist[n];
			//console.log(selected_link.source, n)	
			rect.on("mousedown", (function(nd,n){return function(d){portMouseDown(nd,0,n);}})(selected_link.source, n))
				.on("touchstart", (function(nd,n){return function(d){portMouseDown(nd,0,n);}})(selected_link.source, n))
				.on("mouseup", (function(nd,n){return function(d){portMouseUp(nd,0,n);}})(selected_link.source, n))
				.on("touchend", (function(nd,n){return function(d){portMouseUp(nd,0,n);}})(selected_link.source, n))
				.on("mouseover",function(d) { var port = d3.select(this); port.classed("port_hovered",(mouse_mode!=RED.state.JOINING || mousedown_port_type != 0 ));})
				.on("mouseout",function(d) { var port = d3.select(this); port.classed("port_hovered",false);});
...
console.log("mouse_down", d, portType, portIndex)
		// disable zoom
		...
		if (mouse_mode == RED.state.JOINING && mousedown_node) {
			...
			if (!existingLink) {
				...
				if (!allow_multiple_inputs) {
					// disallow new links to this destination - each input 
					// can have only a single link
			...
			nodeEnter.each(function(d,i) {
				   ...
				   //node.append("rect").attr("class", "node-gradient-bottom").attr("rx", 6).attr("ry", 6).attr("height",30).attr("stroke","none").attr("fill","url(#gradient-bottom)").style("pointer-events","none");
					...
					for (var n=0; n < numInputs; n++) {
						...
						if (!allow_multiple_inputs && (link && link.length > 0)) {
			...
			node.each(function(d,i) {
					...
					if (d.dirty || d.requirementError != undefined) {
						...
						if (d._ports) {
							...
							var x = d.w - 5;
							d._ports.each(function(_,i) {
									var port = d3.select(this);
									port.attr("y",(y+13*i)-5).attr("x",x);
									outputlist[i] = port
									if (!allow_multiple_outputs) {
										var link = RED.nodes.links.filter(function(l) {
											return (l.source == d && l.sourcePort == i);});
										if (link && link.length>0) {
											port.on("mousedown",null)
												.on("touchstart",null)
												.on("mouseup",null)
												.on("touchend",null)
												.on("mouseover",null)
	...
	return {
		...
		cutSelection: function() { copySelection(); deleteSelection(); },
		deleteSelection: deleteSelection,
		paste: function() { importNodes(clipboard,true); },
		selectAll: selectAll,
		...
		removeActiveWorkspace: function() {
			deleteWorkspace(activeWorkspace);
		},
		renameActiveWorkspace: function() {
