...
void *ws_context(ws_cli_conn_t *cli) {
	return cli->context;
}
...
static void *ws_accept(void *data)
{
	...
	for (i = 0; i < MAX_CLIENTS; i++)
	{
		if (client_socks[i].client_sock == -1)
		{
			close(client_socks[i].client_sock);
		}
	}
...
int ws_socket(struct ws_server *ws_srv)
{
	...
	*sock = do_bind_socket(ws_srv);
	...
	if (getsockname(*sock, (struct sockaddr *)&sin, &len) == -1)
		perror("getsockname: ");
	ws_srv->port = ntohs(sin.sin_port);

	/* Wait for incoming connections. */
	printf("[::pid: %d port: %d::]\n", getpid(),ws_srv->port);
	memset(client_socks, -1, sizeof(client_socks));
	for(i=0; i<MAX_CLIENTS; ++i) {
		client_socks[i].context = ws_srv->context;
	}
